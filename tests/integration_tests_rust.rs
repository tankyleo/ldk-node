...
#[tokio::test(flavor = "multi_thread", worker_threads = 1)]
async fn open_channel_variants_reserve_funds_for_optional_anchor_peers() {
	let (bitcoind, electrsd) = setup_bitcoind_and_electrsd();
	let chain_source = random_chain_source(&bitcoind, &electrsd);
	...
	for (variant, node_a, node_b, funding_txo) in opened_with_all_cases {
		...
		assert!(
			balances.total_onchain_balance_sats >= anchor_reserve_sat - reserve_margin_sat
				&& balances.total_onchain_balance_sats <= anchor_reserve_sat + reserve_margin_sat,
			"{} total on-chain balance {} should be close to the anchor reserve {anchor_reserve_sat}",
			variant.label(),
			balances.total_onchain_balance_sats
		);
		assert!(
			balances.total_anchor_channels_reserve_sats >= anchor_reserve_sat - reserve_margin_sat
				&& balances.total_anchor_channels_reserve_sats <= anchor_reserve_sat,
			"{} total anchor reserve {} should be close to {anchor_reserve_sat}",
			variant.label(),
			balances.total_anchor_channels_reserve_sats
		);
		assert!(
			balances.spendable_onchain_balance_sats <= reserve_margin_sat,
			"{} spendable balance {} should be reserved",
			variant.label(),
		...
		assert!(
			channel.channel_value_sats
				> premine_amount_sat - anchor_reserve_sat - reserve_margin_sat
		);
		assert_eq!(channel.counterparty.node_id, node_b.node_id());
